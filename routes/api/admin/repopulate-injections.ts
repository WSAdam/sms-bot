...
function parseAppointment(text: string, refIso: string): string | null {
  ...
  for (const yearOffset of [0, 1]) {
    ...
    const iso =
      `${year}-${String(monthIdx + 1).padStart(2, "0")}-${String(parseInt(day, 10)).padStart(2, "0")}T${String(h).padStart(2, "0")}:${minute}:00-04:00`;
...
export const handler = define.handlers({
  async POST(ctx) {
    ...
    const [convos, history] = await Promise.all([
      db.list(conversationsCollection, { limit: 50_000 }),
      db.list(injectionHistoryCollection, { limit: 50_000 }),
    ]);

    // Phones with prior fired injections — we don't want to schedule a NEW
    // injection for someone we already fired in the past.
    const phonesWithFiredInjection = new Set<string>();
    for (const e of history) {
      const sep = e.id.indexOf("__");
      if (sep > 0) phonesWithFiredInjection.add(e.id.slice(0, sep));
    }
    ...
    for (const e of convos) {
      ...
      const tag = String(m.nodeTag ?? "").toLowerCase();
    ...
    const skipped: Array<{ phone10: string; reason: string; eventTime?: string }> = [];

    for (const a of latestByPhone.values()) {
      const eventTime = parseAppointment(a.message, a.refIso);
      if (!eventTime) {
        skipped.push({ phone10: a.phone10, reason: "could not parse appointment time" });
        continue;
      }
      const eventMs = new Date(eventTime).getTime();
      if (eventMs < nowMs) {
        skipped.push({ phone10: a.phone10, reason: "appointment in the past", eventTime });
        continue;
      }
      const existingPending = await db.get(scheduledInjectionDocPath(a.phone10));
      if (existingPending) {
        skipped.push({ phone10: a.phone10, reason: "already has pending scheduledinjection", eventTime });
        continue;
      }
      if (phonesWithFiredInjection.has(a.phone10)) {
        skipped.push({ phone10: a.phone10, reason: "already fired in injectionhistory", eventTime });
        continue;
      }
      if (dryRun) {
        created.push({ phone10: a.phone10, eventTime, from: a.message.slice(0, 80) });
        continue;
      }
